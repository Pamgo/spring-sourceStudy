...
public class GatewayRateLimitFilterByIp implements GatewayFilter, Ordered {
    ...
    /**
     * 单机网关限流用一个ConcurrentHashMap来存储Bucket,
     * 如果是分布式集群限流的话，可以采用Redis等分布式解决方案
     */
    private static final Map<String, Bucket> LOCAL_CACHE = new ConcurrentHashMap<>();

    /**
     * 桶的最大容量，技能装载 Token 的最大数量
     */
    int capacity;
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        log.debug("IP:{},令牌桶可用的Token数量:{}", ip, bucket.getAvailableTokens());
        if (bucket.tryConsume(1)) {
            return chain.filter(exchange);
        } else {
            // 当可用的令牌数为0时，进行限流返回429状态码
